...
/*?
	Introduction
		...
		*DEVELOPERS:* `Chris van Rensburg`
		...
		The =Uize.Widget.Tree.List= module defines the =Uize.Widget.Tree.List= widget class, a subclass of =Uize.Widget.Tree=.
*/
									...
									_this.wireNode (
										_itemSpecifier + 'TitleLink',
										{
											mouseover:
												function () {
													var
														_tooltipNode = Uize.Node.getById (_tooltip),
														_itemDescription = _item.description
													;
													if (_tooltipNode && _itemDescription) {
														_Uize_Node.setInnerHtml (
															_tooltipNode,
															_Uize_Xml_toAttributeValue (_itemDescription)
														);
														_Uize_Tooltip.showTooltip (_tooltipNode,_true);
													}
												},
											mouseout:function () {_Uize_Tooltip.showTooltip (_this._tooltip,_false)}
										}
									)
								;
							},
