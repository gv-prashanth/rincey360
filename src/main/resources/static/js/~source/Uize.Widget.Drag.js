...
Uize.module ({
	...
	builder:function  (_superclass) {
		...
		/*** Private Instance Methods ***/
			_classPrototype._getDragEventPos = function (_event) {
				if (this._isTouch) {
					var _touch = _event.targetTouches [0];
					return {clientX:_touch.pageX,clientY:_touch.pageY};
				}
				return _event;
			};
		...
		/*** Public Instance Methods ***/
			_classPrototype.initiate = _classPrototype.mousedown = function (_event,_notRelayed) {
				...
				if (!_this._inDrag && _this.get ('enabledInherited')) {
					...
					var _dragEventPos = _this._getDragEventPos (_event);
					_eventStartPos [0] = _eventPos [0] = _eventPreviousPos [0] = _dragEventPos.clientX;
					_eventStartPos [1] = _eventPos [1] = _eventPreviousPos [1] = _dragEventPos.clientY;
					...
					function _fadeDragMoveTo (_endPos,_duration,_fadeProperties) {
						Uize.Fade.fade (_dragMove,[_eventPos [0],_eventPos [1]],_endPos,_duration,_fadeProperties)
					...
					function _handleMoveEvent (_event) {
						if (!_dragIsDone && !_this._dragCancelled) {
							...
							var _dragEventPos = _this._getDragEventPos (_event);
							_dragMove (_dragEventPos.clientX,_dragEventPos.clientY);
						}
					}
					function _cancelDrag (_event) {
						_this.set ({_dragCancelled:_true});
						if (_this._cancelFade && Uize.Fade && Uize.Fade.fade) {
							_fadeDragMoveTo (_eventStartPos,500,_this._cancelFade);
						} else {
		...
		/*** Public Static Methods ***/
			_class.getEventPagePos = function (_event) {
				if (_event.targetTouches) _event = _event.targetTouches [0];
				if ('pageX' in _event) return _event;
				var _documentElement = document.documentElement;
				return {
					pageX:_event.clientX + _documentElement.scrollLeft,
					pageY:_event.clientY + _documentElement.scrollTop
				};
			};
