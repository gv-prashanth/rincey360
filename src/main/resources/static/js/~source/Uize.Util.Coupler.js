...
/*?
	Introduction
		...
		*DEVELOPERS:* `Chris van Rensburg`
		...
		More Examples
			To see an example of how the =Uize.Util.Coupler= class is used, consult the [[../examples/collection-item-coupled-zooming.html][Coupled Zooming Collection Items]] example.
*/
	...
	builder:function (_superclass) {
		...
		/*** Register Properties ***/
			function _updateCoupling () {
				var
					_this = this,
					_couplingHandlers = _this._couplingHandlers
				;

				/*** unwire handlers from any previous coupling ***/
					if (_couplingHandlers) {
						for (
							var _couplingHandlerNo = -1, _couplingHandlersLength = _couplingHandlers.length;
							++_couplingHandlerNo < _couplingHandlersLength;
						) {
							var _couplingHandler = _couplingHandlers [_couplingHandlerNo];
							_couplingHandler._instance.unwire (_couplingHandler._eventName,_couplingHandler._handler)
						}
						_couplingHandlers = _this._couplingHandlers = null;
					}
				...
				if (_this._coupled) {
					...
					if (_instances && _properties) {
						...
						/*** build sync function (if it's not already built), or use previously built one ***/
							if (!_syncFunction) {
								var _functionChunks = ['if (target.UIZE_COUPLER_driver) return;'];
								...
								_functionChunks.push (
									...
									'source.UIZE_COUPLER_driver = 1;',
									'target.set (properties);',
									'delete source.UIZE_COUPLER_driver;'
								);
								_syncFunction = _syncFunctions [_propertiesSignature] =
						...
						/*** wire handlers for coupling instances together ***/
							_couplingHandlers = _this._couplingHandlers = [];
							...
							function _wireCouplingHandler (_controllingInstance,_controlledInstance) {
								...
								_couplingHandlers.push (
									{_instance:_controllingInstance,_eventName:_eventName,_handler:_handler}
								);
