...
/*?
	Introduction
		...
		*DEVELOPERS:* `Chris van Rensburg`
		...
		The =Uize.Data.Simple= module is a package under the =Uize= namespace.
*/
		...
		/*** Public Static Methods ***/
			...
			_package.parse = function (_params) {
				var
					...
					_multiLineValueRegExp = /^(\.{2,}|,{2,}|~{2,}|`{2,}|:{2,})$/,
							...
							if (_multiLineValueRegExp.test (_value)) {
								...
								*/
								...
								while (_lineNo < _linesLengthMinus1) {
									...
									if (
										_multiLineValueRegExp.test (_lineUnindented) &&
								...
								*/
								...
								var _arrayDelimiter = '\\s*\\|\\s*';
								if (_parseName && _value.charAt (0) == '|' && _value.slice (-1) == '|')
									_value = _value
										.replace (new RegExp ('^' + _arrayDelimiter),'')
										.replace (new RegExp (_arrayDelimiter + '$'),'')
										.split (new RegExp (_arrayDelimiter))
