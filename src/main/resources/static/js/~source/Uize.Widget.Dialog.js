...
/*?
	Introduction
		...
		*DEVELOPERS:* `Chris van Rensburg`
		...
		The =Uize.Widget.Dialog= module defines the =Uize.Widget.Dialog= widget class, a subclass of =Uize.Widget=.
*/
			...
			var
				_class = _superclass.subclass (
								...
								/*?
									Instance Properties
										shieldFade
											...
											This property lets you customize the qualities of the opacity fade for the dialog's =shield=, by letting you set values for the set-get properties of the =Uize.Fade= instance (such as =duration=, =acceleration=, =deceleration=, etc.).
								*/
							).wire (
								'Changed.value',
											...
											/*?
												Instance Events
													First Drag Since Shown
														...
														- see the related =Drag End= instance event
											*/
					...
					function () {
						...
						/*** initialization ***/
							_superclass.wire ('Window Resized',function () {_this._updateUiPositionIfShown ()});
					...
					if (_this._autoPosition) {
						...
						if (_mooringNode) {
							...
							_Uize_Node.setStyle (
								_rootNode,
								{
									left:_mooringCoords.left + _this._offsetX,
									top:_mooringCoords.top + _this._offsetY
								}
							);
						} else {
