...
/*?
	Introduction
		...
		*DEVELOPERS:* `Chris van Rensburg`
		...
		The =Uize.Data= module is a package under the =Uize= namespace.
*/
...
Uize.module ({
	...
	builder:function () {
		...
		/*** Public Static Methods ***/
			...
			_package.getLookup = function (_values,_lookupValue) {
				var _lookup = {};
			...
			_package.getReverseLookup = function (_object) {
				var _lookup = {};
				if (typeof _object != _string)
			...
			_package.identical = function (_object1,_object2,_options) {
				...
				function _areIdentical (_object1,_object2) {
					...
					if (_typesMatch && _object1IsObject && _object1 && _object2) {
						...
						} else if (_identical = _object1.constructor == _object2.constructor) {
							if (_object1 instanceof Date) {
			...
			_package.map = function (_mapper,_source,_target) {
				...
				if (typeof _target != 'object')
					_target = _target ? _source : _sourceIsArray ? [] : {}
				;
						...
						Uize.Data.map
							...
							Parameters
								...
								targetARRAYorOBJorBOOL
									...
									- When the *boolean* value =true= is specified (the default value for the =targetARRAYorOBJorBOOL= parameter if it is omitted), then the =Uize.Data.map= method will package the result from the mapper into a new array or object.

									- When the *boolean* value =false= is specified (not the same as not specifying a value), then the =Uize.Data.map= method will package the result from the mapper into the source array or object specified by the =sourceARRAYorOBJorINT= parameter (ie. won't use a different target).

									- When the special value =null= is specified (not the same as not specifying a value), then the =Uize.Data.map= method will not package the result from the mapper into an array or object. This is useful if you just want to use this method as an iterator, without the memory expense of building an array or object.
			...
			_package.reorderArray = function (_source,_reorderScheme,_target) {
				if (typeof _target != 'object')
					_target = _target === _false ? _source : []
				;
				if (_target != _source) _target.length = 0;
				if (_reorderScheme == 'inside out' || _reorderScheme == 'outside in') {
					if (_target == _source) _source = _source.concat ();
					for (
						var
							_targetLength = 0,
							_sourceLengthMinus1 = _source.length - 1,
							_midElementNo = _sourceLengthMinus1 >> 1,
							_insideOut = _reorderScheme == 'inside out',
							_elementNo = _insideOut ? _midElementNo + 1 : -1
						;
						_insideOut ? --_elementNo >= 0 : ++_elementNo <= _midElementNo;
					) {
						_target [_targetLength++] = _source [_elementNo];
						if (_elementNo != _sourceLengthMinus1 - _elementNo)
							_target [_targetLength++] = _source [_sourceLengthMinus1 - _elementNo]
						;
					}
				} else {
					if (_target != _source) Array.prototype.push.apply (_target,_source);
					if (_reorderScheme != 'normal')
						_reorderScheme == 'reverse'
							? _target.reverse ()
							: _target.sort (function () {return Math.random () * 2 - 1})
					;
				}
				return _target;
				/*?
					Static Methods
						Uize.Data.reorderArray
							Returns an array, being a reordered version of the specified source array, using the specified reordering mode.

							SYNTAX
							........................................................................
							reorderedARRAY = Uize.Data.reorderArray (sourceARRAY,reorderingModeSTR);
							........................................................................

							VARIATION 1
							...........................................................................
							reorderedARRAY =
								Uize.Data.reorderArray (sourceARRAY,reorderingModeSTR,targetARRAYorBOOL)
							;
							...........................................................................

							By default, the =Uize.Data.reorderArray= method packages the reordered array elements into a new array and does not modify the source array (ie. it's non-destructive). Specifying the optional =targetARRAYorBOOL= parameter allows you to explicitly specify a target for the operation, into which the reordered elements will be packaged.

							VARIATION 2
							....................................................
							jumbledARRAY = Uize.Data.reorderArray (sourceARRAY);
							....................................................
							...
							Parameters
								sourceARRAY
									A reference to the array that should be reordered.

									This parameter also supports array-like objects, such as collections of DOM nodes.

								reorderingModeSTR
									A string, specifying the reordering mode that should be employed.

									VALUES

									- ='jumbled'= (default) - randomly reorders the array, like shuffling a deck of cards
									- ='reverse'= - reverses the order of the elements
									- ='inside out'= - lays down a new array by starting from the center and diverging out in both directions towards the start and end of the array
									- ='outside in'= - lays down a new array by starting from the start and end of the array and converging in in both directions towards the center
									- ='normal'= - leaves the order of elements unchanged

									Of these modes, ='inside out'= and ='outside in'= may be the hardest to visualize without an example. The statement =Uize.Data.reorderArray ([1,2,3,4,5,6,7,8],'inside out')= would return the resulting array =[4,5,3,6,2,7,1,8]=. Using the value ='outside in'= would instead return =[1,8,2,7,3,6,4,5]=.

								targetARRAYorBOOL
									Lets you specify where the result of the reordering process should be packaged.

									VALUES

									- When the *boolean* value =true= is specified (the default value for the =targetARRAYorBOOL= parameter if it is omitted), then the =Uize.Data.reorderArray= method will package the result of the reordering process into a new array.

									- When the *boolean* value =false= is specified (not the same as not specifying a value), then the =Uize.Data.reorderArray= method will package the result of the reordering process into the source array specified by the =sourceARRAY= parameter (ie. won't use a different target).

									- When an *array* is explicitly specified, then the =Uize.Data.reorderArray= method will package the result of the reordering process into that array. This is convenient if you already have an array into which you wish to package the reordered elements. Incidentally, specifying the value of the =sourceARRAY= parameter as the value for the =targetARRAY= parameter has the same effect as specifying the value =false= for this parameter (ie. use the source as the target, don't use a different target).
				*/
			};
