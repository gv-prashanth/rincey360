...
/*?
	Introduction
		...
		*DEVELOPERS:* `Chris van Rensburg`
		...
		The =Uize.Widget.Resizer= module defines the =Uize.Widget.Resizer= widget class, a subclass of =Uize.Widget=.
*/
...
Uize.module ({
	...
	builder:function  (_superclass) {
		...
		/*** Global Variables ***/
			var
				_ieQuirkyBoxes = Uize.Node.isIe && document.compatMode != 'CSS1Compat',
				_pointIdsMap = {
					northWest:[0,0],
					north:[.5,0],
					northEast:[1,0],
					...
					if (_this._bounds) {
						...
						if (_aspectRatio != null) {
							...
							*/
							...
							_conformedWidth =
								_class.constrain (_width,_this._minWidth,_constrain ? _maxDims [0] : Infinity);
							_conformedHeight =
								_class.constrain (_height,_this._minHeight,_constrain ? _maxDims [1] : Infinity);
						}
						var
							_conformedLeft =
								_constrain ? _class.constrain (_left,_bounds [0],_bounds [2] - _conformedWidth + 1) : _left,
							_conformedTop =
								_constrain ? _class.constrain (_top,_bounds [1],_bounds [3] - _conformedHeight + 1) : _top
						;
						if (
										...
										function _updateAxisCoords (_axis) {
											if (_this._canResizeAxis (_axis,_pointIds)) {
												...
												if (_pointIdForAxis == 'both') {
													...
													if (_this._constrain)
														_newCoords [_coordNo] = _class.constrain (
															_newCoords [_coordNo],
															_bounds [_axis],
															_bounds [_axis + 2]
														)
		...
		/*** Register Properties ***/
			...
			_class.registerProperties ({
				...
				_dragRestTime:{
					name:'dragRestTime',
					onChange:function () {_class.callOn (this.children,'set',[{dragRestTime:this._dragRestTime}])},
					value:250
				},
				_fixedX:{
					name:'fixedX',
