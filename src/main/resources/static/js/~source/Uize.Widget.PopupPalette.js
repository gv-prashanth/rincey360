...
/*?
	Introduction
		...
		*DEVELOPERS:* `Chris van Rensburg`, original code donated by `Zazzle Inc.`
		...
		The =Uize.Widget.PopupPalette= module defines the =Uize.Widget.PopupPalette= widget class, a subclass of =Uize.Widget=.
*/
	...
	builder:function (_superclass) {
					...
					function () {
						...
						/*** create the selector button ***/
							var _selector = _this._selector = _this._addChildButton (
								'selector',
								function (_event) {_this.toggle ('shown')}
							);
							_selector.set ({clickToDeselect:_true});
							_this._updateSelectorSelectedState ();
							function _handleSelectorOverOut (_event) {
								var _isOver = _event.name == 'Over';
								_this._showWhenOver && _isOver && _this.set ({_shown:_true});
								_isOver ? _this._over () : _this._out ()
							}
							_selector.wire ({
		...
		/*** Private Instance Methods ***/
			...
			_classPrototype._shouldTouchTimeouts = function () {
				return this._shown && (this._showWhenOver || this._hideWhenOut);
			};
			_classPrototype._over = function () {this._shouldTouchTimeouts () && this._clearDismissTimeout ()};
			_classPrototype._out = function () {this._shouldTouchTimeouts () && this._setDismissTimeout ()};
					...
					/*** move palette to child of root, if positioning is absolute ***/
						if (_this._positioning == 'absolute') {
							...
							if (_paletteNode && _paletteNode.parentNode != _docBody) {
								...
								*/
								var _paletteWidthBeforeInsertAtRoot = _Uize_Node.getDimensions (_paletteNode).width;
								...
								_this.setNodeStyle (
									_paletteNode,
									{
										...
										right:'',
										bottom:'',
										width:_paletteWidthBeforeInsertAtRoot
									}
								);
							}
						}
						...
						if (_this.isWired) {
							...
							if (_shown) {
								...
								if (_this._positioning == 'absolute') {
									...
									_Uize_Node.setAbsPosAdjacentTo (_this.getNode ('palette'),_this._selector.getNode ());
		...
		/*** Code for Managing Exclusive Display ***/
			...
			if (typeof navigator != 'undefined') {
				...
				document.onmousedown = function (_event) {
					...
					;
					typeof _oldOnmousedown == 'function' && _oldOnmousedown (_event);
				};
			}
