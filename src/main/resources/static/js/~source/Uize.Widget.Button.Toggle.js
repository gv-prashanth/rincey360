...
/*?
	Introduction
		...
		*DEVELOPERS:* `Chris van Rensburg`
		...
		The =Uize.Widget.Button.Toggle= module defines the =Uize.Widget.Button.Toggle= widget class, a subclass of =Uize.Widget.Button=.
*/
			...
			var _updateUiTextAndTitle = _classPrototype._updateUiTextAndTitle = function () {
				...
				if (_this.isWired) {
					...
					if (_valueObject) {
						...
						_text = _class.substituteInto (
							_this._textTemplate,
							_this._textShowNext ? _nextValueObject : _valueObject
						);
						_title = _class.substituteInto (
		...
		/*** Register Properties ***/
			_class.registerProperties ({
				...
				_value:{
					name:'value',
					onChange:function () {
						this._values && this.set ({_valueNo:_class.findRecordNo (this._values,{value:this._value},0)});
					},
					value:_null
				},
				_valueNo:{
