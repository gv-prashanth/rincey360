								...
								/*?
									Instance Properties
										fade
											...
											This property allows access to the fade instance that drives the swap animation. This allows you to modify properties of the animation, such as =duration=, =acceleration=, =deceleration=, and any of the other set-get properties supported by the =Uize.Fade= class.
								*/
							});
									...
									if (_this.isWired) {
										...
										if (_this._previousItem && _this._crossFade) {
											...
											_this._updateItem (
												_this._currentItem,
												_fadeInStartPoint != 1
													? _class.constrain ((_value - 1) / (1 - _fadeInStartPoint) + 1,0,1)
													: _value == 1 ? 1 : 0
											);
											_this._updateItem (
												_this._previousItem,
												_fadeOutEndPoint
													? _class.constrain (1 - _value / _fadeOutEndPoint,0,1)
													: _value ? 0 : 1
											);
										} else {
			...
			_classPrototype._updateItem = function (_item,_value,_updateAllProperties) {
				var
					...
					_styleProperties = _this._dissolve
						? _Uize_Node.getOpacityProperties (_value)
						: _updateAllProperties ? _Uize_Node.getOpacityProperties (1) : {}
				;
