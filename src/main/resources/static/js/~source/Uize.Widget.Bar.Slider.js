					...
					/*** wire up the slider's track ***/
						function _initiateDrag (_event) {
							if (_this.get ('enabledInherited')) {
								_event || (_event = event);
								_calculateCommonCoords ();
								var _trackCoords = _Uize_Node.getCoords (_track);
								function _getScroll (_side) {
									var
										_scrollProperty = 'scroll' + _side,
										_documentElement = document.documentElement
									;
									return (
										Math.max (
											document.body [_scrollProperty],
											_documentElement ? _documentElement [_scrollProperty] : 0
										)
									);
								}
								var _eventPagePos = _Uize_Widget_Drag.getEventPagePos (_event);
								_this._setValueInDrag (
									_valueFunc (
										_scaleFunc (_this.get ('minValue')) +
										_pixelsToScale (
											(_orientationNo ? _eventPagePos.pageY : _eventPagePos.pageX)
