...
/*?
	Introduction
		...
		*DEVELOPERS:* `Chris van Rensburg`
		...
		The =Uize.Fade= module defines the =Uize.Fade= class, a subclass of the =Uize= base class that implements automated interpolation of a simple or compound value, between specified start and end values, and with qualities of the fade - such as duration, curve, quantization, etc. - being configurable through the set-get properties of the class.
	...
	builder:function (_superclass) {
		...
		/*** Public Instance Methods ***/
			...
			_classPrototype.start = function (_properties) {
				this.stop ();
				this.set (_class.copyInto ({_inProgress:true},_properties));
				/*?
					Instance Methods
						start
		...
		/*** Public Static Methods ***/
			var _blendValues = _class.blendValues = function (_value1,_value2,_blend,_quantization,_curve,_previousValue) {
				if (_value1 === _value2) {
					...
					if (_curve && typeof _curve == 'function') {
					...
					if (typeof _value1 == _typeObject && _value1 && !(_value1 instanceof RegExp)) {
						var
							...
							_result = _blendedEqualsPrevious ? _previousValue : _class.isArray (_value1) ? [] : {},
