...
/*?
	Introduction
		...
		*DEVELOPERS:* `Chris van Rensburg`
		...
		The =Uize.String.Lines= module is a package under the =Uize.String= namespace. This module is tested by the =Uize.Test.Uize.String.Lines= unit test module.
	...
	builder:function () {
		...
		/*** Static Methods ***/
			...
			_package.indent = function (_sourceStr,_indentAmount,_indentChars,_indentFirstLine) {
				if (_indentChars == _undefined) _indentChars = '\t';
				if (!_indentAmount || !_indentChars) return _sourceStr;
			...
			_package.split = function (_sourceStr) {
				return (_sourceStr.indexOf ('\r') + 1 ? _sourceStr.replace (/\r\n?/g,'\n') : _sourceStr).split ('\n');
					/* NOTE:
						There's a bizarre issue with the way Microsoft's JScript handles newline and carriage return character matching in regular expressions that are used with the split instance method of JavaScript's String object. Apparently, consecutive \r\n\r\n\r\n resolve down to a single \r\n match, judging by the number of items that are returned in the array. This is problematic, because empty lines are then completely lost - as if they never even existed. Curiously, though, using the same regular expression in the replace method does what one would expect. So, we replace the \r\n with '\n' and then split on the string literal '\n' (rather than a regular expression).
					*/
				/*?
					Static Methods
