...
| /____/ /__/_| | COPYRIGHT : (c)1997-2010 UIZE
...
Uize.module ({
	...
	builder:function (_object) {
		...
		/*** Public Static Methods ***/
			...
			var
				_reordered = [],
				_colorSortFunction = new Function ('colorA','colorB','return colorA.sortValue - colorB.sortValue')
			;
			_object.sort = function (_colors,_referenceColor,_componentWeighting) {
				...
				if (_colorsLength) {
					...
					function _getNormalizedComponent (_tuple,_componentNo) {
						return (
					...
					/*** create reordered array to use in array sort ***/
						var _reorderedItem;
						_reordered.length = _colorsLength;
						for (var _colorNo = _colorsLength; --_colorNo >= 0;) {
							var _dummyColor2Tuple =
								_dummyColor2.from (
									(_reorderedItem = _reordered [_colorNo] || (_reordered [_colorNo] = {}))._colorValue =
										_colors [_colorNo]
								).getTuple (_encoding)
							;
							_reorderedItem.sortValue =
					...
					/*** sort the reordered array ***/
						_reordered.sort (_colorSortFunction);

					/*** rebuild the colors array from the reordered array ***/
						for (var _colorNo = _colorsLength; --_colorNo >= 0;)
							_colors [_colorNo] = _reordered [_colorNo]._colorValue
						;
				}
