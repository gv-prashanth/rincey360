...
/*?
	Introduction
		...
		*DEVELOPERS:* `Chris van Rensburg`
		...
		The =Uize.Wsh= module is a package under the =Uize= namespace and is used by various build scripts, including the auto scruncher and the module reference documentation builder. It will only be of interest to you if you're writing or modifying WSH build scripts.
*/
	...
	builder:function () {
		...
		/*** Public Static Methods ***/
			...
			var _readFile = _package.readFile = function (_params) {
				var
					_file =
						_getFileSystemObject (_params).OpenTextFile (typeof _params == 'string' ? _params : _params.path,1),
					_text = _file.ReadAll ()
				;
				_file.Close ();
				return _text;
			...
			_package.buildFiles = function (_params) {
				...
				_processFolder (_getScriptFolderPath ());
				_writeFile ({
