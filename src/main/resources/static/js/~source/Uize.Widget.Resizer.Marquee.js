...
/*?
	Introduction
		...
		*DEVELOPERS:* `Chris van Rensburg`
		...
		The =Uize.Widget.Resizer.Marquee= module defines the =Uize.Widget.Resizer.Marquee= widget class, a subclass of =Uize.Widget.Resizer=.
*/
					...
					for (var _handleName in _pointIdsMap) {
						if (_handleName != 'move') {
							...
							_Uize_Node.setStyle (
								_handleNode,
								{
									left:_left + _pointIds [0] * _widthMinus1 - (_handleDims.width - 1) / 2,
									top:_top + _pointIds [1] * _heightMinus1 - (_handleDims.height - 1) / 2
								}
							);
						}
					}
							...
							function _initiateDrag (_event) {
								if (_this.get ('enabledInherited')) {
									...
									var
										...
										_eventPagePos = Uize.Widget.Drag.getEventPagePos (_event)
									;
									_this.set ({creatingNew:_true});
									_this.setPositionDuringDrag (
										_eventPagePos.pageX - _shellCoords.left,
										_eventPagePos.pageY - _shellCoords.top,
										_this.get ('minWidth'),
										_this.get ('minHeight')
									);
