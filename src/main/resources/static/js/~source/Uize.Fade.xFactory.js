...
/*?
	Introduction
		...
		*DEVELOPERS:* `Chris van Rensburg`
		...
		The =Uize.Fade.xFactory= module is an extension module that extends the =Uize.Fade= class by adding factory methods for conveniently initiating fades, and by implementing a mechanism for automatically managing a pool of active fade instances.
		...
		/*** Public Static Methods ***/
			_class.fade = function (_target,_startValue,_endValue,_duration,_fadeProperties,_startNow) {
				...
				function _isPropertiesFade (_context,_handler) {
					return _class.isInstance (_context) && _handler == 'set';
				}
				var
				...
				/*** try to find matching active fades in pool (and handle them) ***/
					...
					_fadesToStop && _class.callOn (_fadesToStop,'stop');
						...
						'Changed.inProgress':
							function (_event) {
								...
								_this.get ('inProgress')
									...
									: _fadePool.splice (_class.indexIn (_fadePool,_this),1)
								;
							},
						'Changed.value':
							function (_event) {
								...
								(typeof _handler == 'string' ? _context [_handler] : _handler)
									[_class.isArray (_value) ? 'apply' : 'call'] (_context,_value)
				...
				/*?
					Static Methods
						Uize.Fade.fade
							...
							When the optional =fadePropertiesOBJ= parameter is specified, additional values can be specified for the set-get properties of the =Uize.Fade= class - such as =acceleration=, =deceleration=, =quantization=, etc. These property values will be set on the =Uize.Fade= instance that is created to service the fade process requested by calling this method.
