...
/*?
	Introduction
		...
		*DEVELOPERS:* `Tim Carter`
		...
		The =Uize.Widget.Captcha.Recaptcha= module defines the =Uize.Widget.Captcha.Recaptcha= widget class, a subclass of =Uize.Widget=. The =Uize.Widget.Captcha.Recaptcha= class is a thin wrapper around the Recaptcha object, a js object that is sourced in by reCAPTCHA.

		NOTES
			- For information about the Recaptcha object, which is a non-Uize object provided by the reCAPTCHA project, please visit http://recaptcha.net/apidocs/captcha/client.html#recaptcha-methods.
	...
	builder:function (_superclass) {
		...
		/*** Public Instance Methods ***/
			_classPrototype.initializeCaptcha = function () {
				...
				if (!(_this.recaptchaObject = window.Recaptcha))
			...
			_classPrototype.validate = function ( _params ) {
				...
				_this._commObject.request ({
					...
					callback:function (_response) {
						...
						_callback && typeof _callback == 'function' && _callback ( _response );
					}
				});
				/*?
					Instance Methods
