...
/*?
	Introduction
		...
		*DEVELOPERS:* `Chris van Rensburg`
		...
		The =Uize.Widget.Bevel= module defines the =Uize.Widget.Bevel= widget class, a subclass of =Uize.Widget=.
*/
			...
			_classPrototype._updateUi = function () {
				...
				if (_node) {
					...
					var
						...
						_html =
							'<div style="width:' + (_this._imageDims [0] - _boxModelDimOffset) + 'px; height:' + (_this._imageDims [1] - _boxModelDimOffset) + 'px; border-left:' + _borderCommon + '#ccc; border-top:' + _borderCommon + '#fff; border-right:' + _borderCommon + '#444; border-bottom:' + _borderCommon + '#000;' + _Uize_Node.getOpacityStr (_this._opacity) + '">' +
							'&nbsp' +
							'</div>'
					;
