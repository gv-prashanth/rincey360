...
/*?
	Introduction
		...
		*DEVELOPERS:* `Chris van Rensburg`
		...
		The =Uize.Node= module is a package under the =Uize= namespace.
...
Uize.module ({
	...
	builder:function () {
		...
		/*** Utility Functions ***/
			...
			function _getDocumentScrollPos () {
				var _documentElement = document [_isSafari ? 'body' : 'documentElement'];
				return {
					scrollLeft:_documentElement.scrollLeft,
		...
		/*** Public Static Methods ***/
			...
			var _doForAll = _package.doForAll = function (_nodeBlob,_function,_idPrefix,_nodeCache) {
				...
				if (_nodeBlob != _undefined) {
					if (_isNode (_nodeBlob)) {
						...
						if (
							(_typeofNodeBlob == 'object' || _typeofNodeBlob == _typeFunction) &&
			...
			_package.find = function (_properties) {
				/*** return early if the parameter for find is already resolved to a node or array of nodes ***/
					if (
						typeof _properties != 'object' || !_properties ||
						...
						if (_isMatch = _root ? _isOnNodeTree (_node,_root) : _true) {
							for (var _propertyName in _unusedProperties) {
								var
									...
									_typeofTest = typeof _test
								;
								if (
									!(
										_simpleTypesMap [_typeofTest]
											? _nodePropertyValue == _test
											: (
												_test instanceof RegExp
													? _test.test (_nodePropertyValue || '')
													: (
														_typeofTest == _typeFunction
															? _test.call (_node,_nodePropertyValue)
															: _true
													)
											)
									)
			...
			var _getCoords = _package.getCoords = function (_node) {
				var
					...
					_height = 0,
					_seen = _true,
					_docScrollPos = _getDocumentScrollPos (),
					_windowDims = _getDimensions (window)
				;
				function _factorInDocScroll () {
					_x += _docScrollPos.scrollLeft;
					_y += _docScrollPos.scrollTop;
				}
				if (_node == window) {
					_factorInDocScroll ();
					...
					/*** test if node is in portion of document scrolled into view ***/
						if (_seen)
							_seen = _doRectanglesOverlap (
								_x,_y,_width,_height,
								_docScrollPos.scrollLeft,_docScrollPos.scrollTop,_windowDims.width,_windowDims.height
							)
						;
				}
				return {
			...
			_package.getEffectiveBgColor = function (_node) {
				...
				while ((!_background || _background == 'transparent' || _background == 'none') && _node) {
					_background = _getStyle (_node,'backgroundColor');
				...
				/*?
					Static Methods
						Uize.Node.getEffectiveBgColor
							...
							SYNTAX
							...............................................................
							styleBgColorSTR = Uize.Node.getEffectiveBgColor (nodeSTRorOBJ);
							...............................................................
			...
			var _getOpacityProperties = _package.getOpacityProperties = function (_opacity) {
				return _isIe ? {filter:'alpha(opacity=' + Math.round (_opacity * 100) + ')'} : {opacity:_opacity + ''};
				/*?
					Static Methods
						Uize.Node.getOpacityProperties
							...
							SYNTAX
							...............................................................................
							styleOpacityPropertiesOBJ = Uize.Node.getOpacityProperties (opacityFLOATorOBJ);
							...............................................................................
							...
							- see also the =Uize.Node.getOpacityStr= and =Uize.Node.setOpacity= static methods
				*/
			};
			...
			_package.getOpacityStr = function (_opacity) {
				...
				/*?
					Static Methods
						Uize.Node.getOpacityStr
							Returns a string, representing the CSS style property corresponding to the specified opacity value.

							SYNTAX
							....................................................
							opacitySTR = Uize.Node.getOpacityStr (opacityFLOAT);
							....................................................

							This method abstracts the difference between Microsoft Internet Explorer (that supports the proprietary =alpha= option for the =filter= style property) and browsers that support the W3C standard =opacity= style property. For example, the expression =Uize.Node.getOpacityStr (.5)= will produce the string ='filter:alpha(opacity=50);'= for Internet Explorer, and the string ='opacity:0.5;'= for all other browsers.

							NOTES
							- see also the =Uize.Node.getOpacityProperties= and =Uize.Node.setOpacity= static methods
				*/
			};
				...
				/*?
					Static Methods
						Uize.Node.injectHtml
							...
							VARIATION
							..............................................................
							Uize.Node.injectHtml (nodeBLOB,htmlSTRorOBJ,injectionModeSTR);
							..............................................................
			...
			_package.getEventAbsPos = function (_event) {
				var _docScrollPos = _getDocumentScrollPos ();
				return {
					left:(_event || _mousePos).clientX + _docScrollPos.scrollLeft,
					top:(_event || _mousePos).clientY + _docScrollPos.scrollTop
				};
				/*?
					Static Methods
				...
				/*?
					Static Methods
						Uize.Node.setOpacity
							...
							- this method can operate on multiple nodes at a time. For more details, see the section on `Node Blob`
							- the =opacityFLOATorOBJ= parameter can be an object that implements a =valueOf= interface (such as an instance of a =Uize= subclass that implements the =value= set-get property)
							- see also the =Uize.Node.getOpacityStr= and =Uize.Node.getOpacityProperties= static methods
				*/
			};
					...
					function (_node) {
						...
						for (var _property in _properties)
							_nodeStyle [_property] =
								(
									typeof (_propertyValue = _properties [_property]) == 'object' && _propertyValue
				...
				/*?
					Static Methods
						Uize.Node.stylePropertiesAsStr
							...
							SYNTAX
							....................................................
							Uize.Node.stylePropertiesAsStr (stylePropertiesOBJ);
							....................................................
			...
			_package.wireEvent = // DEPRECATED (see documentation)
			_package.wireEvents = // DEPRECATED (see documentation)
			_package.wire = function (_nodeBlob,_eventName,_handler,_wiringOwnerId) {
				...
				;
				if (typeof _eventName == 'object' && !(_isVirtualDomEvent = !!_eventName.virtualDomEvent)) {
		...
		/*?
			Deprecated Features
				Uize.Node.wireEvent (static method) -- DEPRECATED 2009-11-15
					This method has been deprecated in favor of the more versatile and more concisely named =Uize.Node.wire= static method.

					INSTEAD OF...
					.............................................................
					Uize.Node.wireEvent (nodeBLOB,eventNameSTR,eventHandlerFUNC);
					.............................................................
				...
				Uize.Node.wireEvents (static method) -- DEPRECATED 2009-11-15
					This method has been deprecated in favor of the more versatile and more concisely named =Uize.Node.wire= static method.
